    ...
    <Card className="space-y-0 overflow-hidden rounded-2xl border border-border/70 shadow-sm">
      ...
      <div className="grid gap-4 p-6 md:grid-cols-4">
        ...
        <div className="space-y-1 rounded-2xl border border-border/60 p-4">
          <StatLabel label="时间范围" />
          <p className="text-base font-medium text-foreground">{formatRange(iteration.start_date, iteration.end_date)}</p>
        </div>
        <div className="space-y-1 rounded-2xl border border-border/60 p-4">
          <StatLabel label="待执行 SQL" />
          <Badge tone={sqlTone(iteration.pending_sql_count)} variant="soft" className="px-3 py-1 text-base font-medium">
            {iteration.pending_sql_count ?? 0} 条
          </Badge>
        </div>
        <div className="space-y-2">
