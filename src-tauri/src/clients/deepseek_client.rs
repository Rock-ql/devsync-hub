...
7. 一级条目（需求名称、项目名、"其他工作"）必须使用Markdown加粗（**文字**），二级及以下条目不加粗。
...
7. 对同一项目下的工作项，只合并"围绕同一功能点的冗余或进度性描述"，对不同功能方向的工作保留为独立条目（每个项目 1-6 条）；避免逐条复制原始提交消息，但不强制将不同方向的工作压缩成一句话。
8. 每条工作项应体现工作成果或效果，而非仅描述动作。例如"完成XX功能开发并通过测试"优于"开发XX功能"。
9. 一级条目（项目名）必须使用Markdown加粗（**文字**），二级及以下条目不加粗。
