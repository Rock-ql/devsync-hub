...
4. 下周工作计划：根据本周工作内容智能推导下周计划，按项目分组，每个项目 1-3 条。推导逻辑如下：
   - 开发中的功能 → 继续开发/联调/自测
   - 修复中的问题 → 回归验证/优化完善
   - 联调完成的功能 → 提测/配合测试
   - 测试中的功能 → 配合测试/修复测试问题
   - 新启动的工作 → 继续推进/完成开发
   - 已上线的功能 → 线上监控/问题跟进（仅在有必要时列出）
5. 模板仅作为格式参考，禁止拷贝模板中的历史业务条目。
6. 一级和二级条目都使用阿拉伯数字编号。
7. 若输入已将多个仓库/模块归并到同一项目名，输出时必须保持该归并，不可再拆分。
8. 对同一项目下的工作项，只合并"围绕同一功能点的冗余或进度性描述"，对不同功能方向的工作保留为独立条目（每个项目 1-6 条）；避免逐条复制原始提交消息，但不强制将不同方向的工作压缩成一句话。
9. 每条工作项应体现工作成果或效果，而非仅描述动作。例如"完成XX功能开发并通过联调验证"优于"开发XX功能"。
10. 禁止使用Markdown加粗语法（**文字**），所有条目均使用纯文本。
...
- 下周工作计划：根据本周各项目的工作进展，智能推导下周计划，按项目分组，每个项目 1-3 条。
- 对同一项目下的工作项：只合并"同一功能点的冗余描述"，对不同功能方向的工作保留独立条目（每个项目 1-6 条），用简洁中文概括各工作的完成效果。
