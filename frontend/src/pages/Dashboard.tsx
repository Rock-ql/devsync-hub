  ...
  const stats = [
    {
      ...
      total: data?.projectCount || 0,
      icon: FolderGit2,
      subValue: null,
    },
    {
      name: '进行中迭代',
      value: data?.activeIterationCount || 0,
      total: data?.iterationCount || 0,
      icon: IterationCw,
      subValue: null,
    },
    {
    ...
    {
      name: '本周提交',
      value: data?.weekCommitCount || 0,
      subValue: `今日 ${data?.todayCommitCount || 0}`,
      icon: GitCommit,
    },
  ]
      ...
      >
        ...
        <motion.div variants={fadeInUp}>
          <Card className="h-full">
            ...
            <CardContent className="space-y-3">
              {data?.pendingSqlByProject?.length ? (
                data.pendingSqlByProject.map((item) => (
                  <div
                    key={item.projectId}
                    className="flex items-center justify-between rounded-xl border border-border/60 bg-background/60 px-4 py-3 transition hover:bg-muted/60"
                  >
