...
export default function Settings() {
  ...
  const updateSettingMutation = useMutation({
    mutationFn: (data: Record<string, string>) => api.post('/setting/batch-update', data),
    onSuccess: () => {
  ...
  const { data: apiKeys, isLoading: apiKeysLoading } = useQuery<ApiKey[]>({
    queryKey: ['apiKeys'],
    queryFn: () => api.get('/apikey/list'),
  })

  const createKeyMutation = useMutation({
    mutationFn: (name: string) => api.post<{ keyValue: string }>('/apikey/create', { name }),
    onSuccess: (data) => {
  ...
  const deleteKeyMutation = useMutation({
    mutationFn: (id: number) => api.post(`/apikey/delete/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['apiKeys'] })
    },
  })
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {key.keyPrefix}••••••••
                            {key.lastUsedAt && ` · 最后使用: ${key.lastUsedAt.split('T')[0]}`}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
